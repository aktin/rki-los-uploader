...
case_data <- lapply(file_numbers, function(i) {
  file_path <- sprintf("~/Documents/BMG_Teichert/export_1887/%d_result/case_data.txt", i)
...
los<-los%>%filter(np_prozent<20)
los<-los%>%filter(mean<300)
...
gesamt_db_Pand<-gesamt_db_Pand%>%filter(klinik !=33)
...
df<-fallzahl %>% 
  group_by(Var1, Var2) %>% 
  summarise(mean_Fallzahl = mean(Freq, na.rm = TRUE))
df<-df%>%filter(mean_Fallzahl !=0)
...
zeitraum<-zeitraum%>%filter(KW != "21" & KW!="26")
...
write.table(KW_22_25_2023,file = "LOS_2023-W22_to_2023-W25_20230629-094752.csv",dec=".",sep = ",",row.names = FALSE,quote=FALSE)
