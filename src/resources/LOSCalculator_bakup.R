...
processFiles <- function(exDir, file_numbers) {
  ...
  for (i in file_numbers) {
    ...
    if (file.exists(filepath_i)) {
      ...
      errors_clinic_i <- 0
      errors_clinic_i <- errors_clinic_i + sum(is.na(df$entlassung_ts))
      ...
      errors_clinic_i <- errors_clinic_i + sum(is.na(df$aufnahme_ts))
      df <- df[complete.cases(df$aufnahme_ts), ]  # remove rows where aufnahme_ts is still empty
      df <- df %>% mutate(aufnahme_ts = ifelse(triage_ts < aufnahme_ts, triage_ts, aufnahme_ts))  # replace aufnahme_ts with triage_ts if triage_ts is prior to aufnahme_ts
      
      all_data_df <- rbind(all_data_df, df)
      error_rates <- rbind(error_rates, list(clinic = i, errors = errors_clinic_i))
  ...
  if(nrow(all_data_df) > 0) {
    return(list(case_data = all_data_df, error_rates = na.omit(error_rates)))  
  } else {
    return(NULL)
  }
...
filterLosValid <- function(los, num_of_cases, error_rates) {
  los <- left_join(los, num_of_cases, by="clinic")
  for(i in 1:nrow(los)) {
    los$Freq[los$clinic==i] <- los$Freq[los$clinic==i] + error_rates$errors[error_rates$clinic==i]
  }
...
main <- function(){
    ...
    processed <- processFiles(exDir, file_numbers)
    case_data <- processed$case_data
    print(case_data)
    error_rates <- processed$error_rates
    if(!is.null(case_data)) {
      timeframe <- performAnalysis(case_data, error_rates)
    } else {
